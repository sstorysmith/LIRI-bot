...
function spotifyThis(input) {
    if (input === "") {
        //If no song input then default to "The Sign" per requirements
        input = "The Sign";
    }
...
function concertThis(input) {
    axios({
        ...
        console.log(
            "Venue: ",
            response.data[0].venue.name, //only first concert shown
            "\r\nLocation: ",
            response.data[0].venue.country,
            "\r\nDate: ",
            moment(response.data[0].datetime).format("dddd, MMMM Do YYYY, h:mm:ss a")
        );
    });
}
